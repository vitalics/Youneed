...
export interface SchedulerHost {
  flush(): void; // render now
  readonly depth: number; // tree depth — for parent-before-child ordering
}
...
export function createScheduler(): Scheduler {
  const pending = new Map<SchedulerHost, Priority>();
  let microQueued = false;
  let idleQueued = false;

  const flushBucket = (wanted: Priority) => {
    let guard = 0;
    for (;;) {
      const hosts: SchedulerHost[] = [];
      ...
      if (++guard > 1000) throw new Error("scheduler: flush did not converge");
    }
  };
  ...
  return {
    ...
    request(host, priority) {
      ...
      const next: Priority =
        current === "render-blocking" || priority === "render-blocking"
          ? "render-blocking"
          : "background";
...
let defaultScheduler: Scheduler = createScheduler();

/** Swap the global scheduler (e.g. a synchronous one for SSR). */
export function setDefaultScheduler(scheduler: Scheduler): void {
 ...
 */
export function createFpsScheduler(fps?: number): Scheduler {
  ...
  const schedule = (cb: (t: number) => void): number =>
    hasRaf ? requestAnimationFrame(cb) : (setTimeout(() => cb(now()), minInterval || 16) as unknown as number);
  const unschedule = (id: number): void =>
    hasRaf ? cancelAnimationFrame(id) : clearTimeout(id);

  let frameId: number | undefined; // the in-flight frame/timer handle (if any)
  let looping = false;
  let last = -Infinity;
  ...
  const tick = (t: number) => {
    ...
    if (t - last < minInterval) return; // cap not reached yet -> skip this rAF
    const dt = last === -Infinity ? 0 : t - last;
    last = t;
    if (frames.size) for (const cb of [...frames]) cb(dt); // game tick -> may dirty hosts
  ...
  const stop = () => {
    if (frameId !== undefined) unschedule(frameId);
    frameId = undefined;
    looping = false;
    last = -Infinity;
    pending.clear();
...
export const rafScheduler: Scheduler = createFpsScheduler();
