...
export function topology(servers) {
    return { servers };
}
...
export function fromApp(app, meta) {
...
const has = (mw, name) => !!mw?.some((m) => m.toLowerCase().includes(name));
...
export function securityAudit(server) {
    const out = [];
    const mw = server.middleware ?? [];
    const add = (f) => void out.push({ ...f, docs: OWASP_DOCS });
    ...
    for (const r of server.routes) {
        if (r.internal)
            continue; // infra route (e.g. the devtools endpoint) — not audited
...
export function auditGrade(findings) {
...
const guardCount = (r) => r.guardNames?.length ?? r.guards ?? 0;
...
export function toOpenApi(server, options = {}) {
    const paths = {};
    let anyGuards = false;
    for (const r of server.routes) {
        if (r.kind && r.kind !== "http")
            continue; // ws/sse aren't OpenAPI
        if (r.internal)
            continue; // infra routes (devtools endpoint) aren't part of the API
        ...
        const responses = r.schema?.response
...
export function microbench(fn, options = {}) {
...
export async function microbenchAsync(fn, options = {}) {
