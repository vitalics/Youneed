...
import { rafScheduler, getDefaultScheduler, } from "@youneed/dom-scheduler";
import { compileTemplate, bindParts, appendSlot, getStyles, getLazyStyles, loadLazySheet, normalizeStyles, toStyleSheets, externalProps, setCurrentHost, getCurrentHost, EventPart, } from "./template.js";
import { createSignal, createComputed, createEffect } from "./signals.js";
import { task } from "./task.js";
import { getReactiveProps, getExposedEvents, attrPropMap, reflectPropMap, getWatchers, rendersCompiled, define, propDecorator, eventDecorator, watchDecorator, defineDecorator, compileDecorator, computedDecorator, } from "./decorators.js";
function flushSync() {
...
const hydrationData = new WeakMap();
/** Props an element was created with (for SSR serialization). */
function getHydrationProps(el) {
...
function reactive(Base) {
    class Reactive extends Base {
        ...
        #styleRules() {
            if (!this.#usesShadow)
                return []; // light DOM has no adoptedStyleSheets
            const out = [];
        ...
        #collectListeners() {
            const template = (this.#parts ?? [])
                .filter((p) => p instanceof EventPart)
                .map((p) => ({
                type: p.name,
        ...
        #devtools(kind, emit) {
            ...
            if (this.constructor.devtools === false)
                return;
                ...
                if (attrs)
                    for (const attr in attrs)
                        if (this.hasAttribute(attr))
                            this.#reflectAttr(attrs[attr], this.getAttribute(attr));
        ...
        #loadLazyStyles() {
            if (!this.#usesShadow)
                return; // no adoptedStyleSheets target in light DOM
        ...
        [Symbol.dispose]() {
            ...
            this.#disposed = true;
            for (const teardown of this.#cleanups.splice(0))
                teardown();
            this.#controller.abort(); // auto-removes listeners registered with this.abortSignal
        }
        requestUpdate(priority = this.constructor
            ...
            Object.defineProperty(this, name, {
                ...
                set: (value) => {
                    ...
                    if (reflectAttr !== undefined)
                        this.#writeAttr(reflectAttr, value);
                    this.requestUpdate();
                },
            });
        }
        ...
        #renderInner() {
            ...
            try {
                ...
                if (firstRender)
                    this.#mounted = true; // ensure mounted even if onMount threw
...
function Component(tagName, baseOrOptions, options) {
    ...
    if (opts.priority !== undefined)
        Scoped.priority = opts.priority;
    if (opts.scheduler !== undefined)
        Scoped.scheduler = opts.scheduler;
    if (opts.shadow !== undefined)
        Scoped.shadow = opts.shadow;
...
function Mount(root, Root) {
    ...
    if (!Root.tagName)
        throw new Error("Mount: component has no tag name");
    define(Root);
