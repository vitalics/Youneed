...
export interface CorsOptions {
  ...
  methods?: string[];
  allowedHeaders?: string[]; // default: reflect the request's ACR-Headers
...
export function cors(opts: CorsOptions = {}): Middleware {
  ...
  const resolveOrigin = (origin: string): string => {
    ...
    if (rule === true) return origin || "*";
    if (rule === "*") return opts.credentials ? origin || "*" : "*";
    if (typeof rule === "string") return rule;
    if (Array.isArray(rule)) return rule.includes(origin) ? origin : "";
    if (typeof rule === "function") return rule(origin) ? origin : "";
    return "";
  };
  return (ctx, next) => {
    ...
    if (allow) {
      ...
      if (allow !== "*") res.setHeader("Vary", vary(res, "Origin"));
    }
    ...
    if (req.method === "OPTIONS" && req.headers["access-control-request-method"]) {
      ...
      if (opts.maxAge !== undefined) res.setHeader("Access-Control-Max-Age", String(opts.maxAge));
