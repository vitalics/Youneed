...
export class Schema<T> {
  /** @internal */
  constructor(private readonly def: SchemaDef<T>) {}
  ...
  validate(value: T): void {
    for (const v of this.def.validators) v(value);
  }
  ...
  parse(input: unknown): ParseResult<T> {
    if (input === undefined || input === null) {
      if (this.def.hasDefault) return { success: true, value: this.def.default as T };
      if (this.def.optional) return { success: true, value: undefined as T };
  ...
  #accept(input: unknown): T {
    if (typeof input === "string") return this.def.coerce(input);
    switch (this.def.kind) {
      case "number":
        if (typeof input === "number" && Number.isFinite(input)) return input as T;
        throw new Error("must be a finite number");
      case "int":
        if (typeof input === "number" && Number.isInteger(input)) return input as T;
        throw new Error("must be an integer");
      case "port":
        if (typeof input === "number" && Number.isInteger(input) && input >= 1 && input <= 65535) return input as T;
  ...
  refine(test: (value: T) => boolean, message: string): Schema<T> {
    const v: Validator = (value) => {
      if (!test(value as T)) throw new Error(message);
    };
    return this.clone<T>({ validators: [...this.def.validators, v] });
  }
}

/** Inclusive bound validator — length for strings, value for everything numeric. */
function bound(kind: Kind, dir: "min" | "max", n: number): Validator {
  const isStr = kind === "string";
  return (value) => {
    const measure = isStr ? (value as string).length : (value as number);
    if (dir === "min" && measure < n) throw new Error(isStr ? `length must be >= ${n}` : `must be >= ${n}`);
    if (dir === "max" && measure > n) throw new Error(isStr ? `length must be <= ${n}` : `must be <= ${n}`);
  };
}

function make<T>(kind: Kind, coerce: Coerce<T>, validators: Validator[] = []): Schema<T> {
  return new Schema<T>({ kind, coerce, validators, optional: false, hasDefault: false, secret: false });
}
 ...
 *  property names. */
export const t = {
  string: (): Schema<string> => make<string>("string", (raw) => raw),
  number: (): Schema<number> =>
    make<number>("number", (raw) => {
      const n = Number(raw);
      if (raw.trim() === "" || !Number.isFinite(n)) throw new Error("must be a finite number");
      return n;
    }),
  int: (): Schema<number> =>
    make<number>("int", (raw) => {
      const n = Number(raw);
      if (raw.trim() === "" || !Number.isInteger(n)) throw new Error("must be an integer");
      return n;
    }),
  boolean: (): Schema<boolean> =>
    make<boolean>("boolean", (raw) => {
  ...
  port: (): Schema<number> =>
    make<number>("port", (raw) => {
      const n = Number(raw);
      if (raw.trim() === "" || !Number.isInteger(n) || n < 1 || n > 65535) throw new Error("must be a port (1..65535)");
      return n;
    }),
  url: (): Schema<string> =>
    make<string>("url", (raw) => {
  ...
  enum: <const V extends readonly string[]>(values: V): Schema<V[number]> =>
    make<V[number]>("enum", (raw) => {
      if (!values.includes(raw as V[number])) throw new Error(`must be one of: ${values.join(", ")}`);
...
// ── env engine (shared by the platform packages) ────────────────────────────────
...
export type EnvOf<Sc extends EnvSchema> = { [K in keyof Sc]: Infer<Sc[K]> };
...
export function parseEnv<Sc extends EnvSchema>(
  source: EnvSource,
  schema: Sc,
): { values: EnvOf<Sc>; issues: EnvIssue[] } {
  ...
  for (const key of Object.keys(schema)) {
    ...
    if (!present) {
      if (s.hasDefault) {
        try {
          s.validate(s.defaultValue as never);
  ...
  return { values: out as EnvOf<Sc>, issues };
}

/** A safe-to-log view of a validated env, masking every `.secret()` key. */
export function describeEnv<Sc extends EnvSchema>(values: EnvOf<Sc>, schema: Sc): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const key of Object.keys(schema)) {
    out[key] = schema[key]!.isSecret ? "[REDACTED]" : (values as Record<string, unknown>)[key];
  }
  return out;
}
