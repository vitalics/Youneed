...
function traceIdOf(state) {
    const span = state.span;
    if (span?.traceId && span.traceId !== ZERO_TRACE)
        return span.traceId;
    return undefined;
}
/**
...
export function log(ctx) {
    return ctx.state[ctx.state.__loggerKey] ?? fallbackLogger();
}
/**
 ...
 */
export function logger(base, opts = {}) {
    const stateKey = opts.stateKey ?? DEFAULT_STATE_KEY;
    return (ctx, next) => {
        const bindings = { requestId: ctx.requestId };
        const traceId = traceIdOf(ctx.state);
        if (traceId)
            bindings.traceId = traceId;
        if (opts.bindings)
            Object.assign(bindings, opts.bindings(ctx));
 ...
 * fresh `createLogger()`.
 */
export function loggerProvider(base, opts = {}) {
        ...
        install(instance) {
            Object.defineProperty(instance, key, {
                get() {
                    const ctx = context();
                    if (!ctx)
                        return logger; // outside a request (WS frame, startup) → base
                    const stateKey = ctx.state.__loggerKey ?? opts.stateKey ?? DEFAULT_STATE_KEY;
                    return ctx.state[stateKey] ?? logger;
                },
                enumerable: false,
                configurable: true,
            });
        },
        __contributes: undefined,
    };
}
